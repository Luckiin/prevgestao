...
export default function QuemSomos() {
  ...
  return (
    <section id="quem-somos" style={{ background: "#070102", padding: "120px 0", borderTop: "1px solid rgba(201,169,110,.06)" }}>
      <div style={{ maxWidth: 1280, margin: "0 auto", padding: "0 32px" }}>
        ...
        <div ref={el => refs.current[0] = el} style={{ maxWidth: 900, marginBottom: 80 }}>
          ...
          <h2 style={{ fontFamily: "'Playfair Display', Georgia, serif", fontSize: "clamp(32px, 4vw, 56px)", fontWeight: 700, color: "#f5f0e8", lineHeight: 1.15, letterSpacing: "-0.02em", marginBottom: 28 }}>
            Cada trabalhador merece receber o benefício ao qual tem direito.{" "}
            <span style={{ color: "#C9A96E" }}>Nós garantimos isso.</span>
          </h2>
          <p style={{ fontSize: 16, lineHeight: 1.8, color: "rgba(245,240,232,.5)", maxWidth: 680 }}>
            ...
            O escritório Deise Lisboa Advocacia existe para ser essa diferença.
          </p>
        </div>

        <div style={{ height: 1, background: "linear-gradient(90deg, rgba(201,169,110,.25), transparent)", marginBottom: 80 }} />

        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(280px, 1fr))", gap: 48 }}>
          {PILARES.map((p, i) => (
            <div key={p.num} ref={el => refs.current[i + 1] = el}>
              <div style={{ fontFamily: "'Playfair Display', Georgia, serif", fontSize: 64, fontWeight: 700, lineHeight: 1, color: "rgba(201,169,110,.12)", marginBottom: 24, userSelect: "none" }}>{p.num}</div>
              <h3 style={{ fontFamily: "'Playfair Display', Georgia, serif", fontSize: 20, fontWeight: 600, color: "#f5f0e8", marginBottom: 16, lineHeight: 1.3 }}>
                {p.title}
              </h3>
              <p style={{ fontSize: 14, lineHeight: 1.8, color: "rgba(245,240,232,.45)" }}>
                {p.desc}
              </p>
            </div>
          ))}
